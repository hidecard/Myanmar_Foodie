...
class BotHandlers {
    ...
    async start(ctx) {
        ...
        // Get user info with role (user is created/updated by middleware)
    ...
    // Add shop (shop admin command)
    async addShop(ctx) {
        const user = await database.getUser(ctx.from.id);
        
        if (user.role !== 1 && user.role !== 2) {
            await ctx.reply('❌ ဒီအမိန့်ကို Shop Admin များသာ အသုံးပြုနိုင်ပါသည်။');
            return;
        }

        await this.startShopCreation(ctx);
    }

    // View my shops (shop admin command)
    async viewMyShops(ctx) {
        const user = await database.getUser(ctx.from.id);
        ...
        const shops = await database.getShopsByOwner(user.telegram_id);
        
        if (shops.length === 0) {
            await ctx.reply('📝 သင့်ဆိုင်များ မရှိပါ။\n\nဆိုင်အသစ်ထည့်ရန် /add_shop ကိုအသုံးပါ။');
            return;
        }

        let message = '🏪 *ကျွန်ုပ်ဆိုင်များ*\n\n';
    ...
    async addMenu(ctx) {
        ...
        if (shops.length === 0) {
            await ctx.reply('❌ ပထမဆုံးဆိုင်တစ်ခုထည့်ပေးပါ။\n/add_shop ကိုအသုံးပြီးဆိုင်ထည့်ပါ။');
            return;
        }

        // Start menu creation wizard
        ctx.session.menuStep = 'select_shop';
        ctx.session.menuData = {};

        let message = '📋 *Menu ထည့်ရန်*\n\n';
        message += 'ဘယ်ဆိုင်မှာ menu ထည့်မည်နည်း?\n\n';
        
        shops.forEach((shop, index) => {
            message += `${index + 1}. ${shop.name} (${shop.township})\n`;
        });
        
        message += '\nဆိုင်နံပါတ်ကိုရိုက်ပြပါ:';
    ...
    async handleMenuCreation(ctx) {
        if (!ctx.session.menuStep) return;

        const step = ctx.session.menuStep;
        
        switch(step) {
            case 'select_shop':
                const shopId = parseInt(ctx.message.text);
                const user = await database.getUser(ctx.from.id);
                const shops = await database.getShopsByOwner(user.telegram_id);
                
                const shop = shops.find(s => s.id === shopId);
                if (!shop) {
                    await ctx.reply('❌ မှားနေသော ဆိုင်နံပါတ်ဖြစ်ပါသည်။ ပြန်ရိုက်ပါ:');
                    return;
                }
                
                ctx.session.menuData.shop_id = shopId;
                ctx.session.menuStep = 'item_name';
                
                await ctx.reply('✅ ဆိုင်ရွေးချယ်ပြီးပါပြီ။\n\n📝 Menu item အမည်ကိုရေးပေးပါ:');
                break;
                
            case 'item_name':
                ctx.session.menuData.item_name = ctx.message.text.trim();
                ctx.session.menuStep = 'price';
                
                await ctx.reply('💰 စျေးနှုန်း (MMK) ကိုရေးပေးပါ:');
                break;
                
            case 'price':
                const price = parseInt(ctx.message.text);
                if (isNaN(price) || price <= 0) {
                    await ctx.reply('❌ မှားနေသော စျေးနှုန်းဖြစ်ပါသည်။ ပြန်ရေးပါ:');
                    return;
                }
                ...
                await ctx.reply('📝 ဖော်ပြချက် (optional) ကိုရေးပေးပါ:');
                break;
                
            case 'description':
                ctx.session.menuData.description = ctx.message.text.trim();
                ctx.session.menuStep = 'category';
                
                await ctx.reply('🏷️ Category (optional) ကိုရေးပေးပါ:\nဥပမာ: Main Course, Drink, Dessert');
                break;
                
            case 'category':
                ctx.session.menuData.category = ctx.message.text.trim();
            ...
            case 'photo':
                if (ctx.message.text.toLowerCase() === 'skip' || ctx.message.text.toLowerCase() === 'ခုံချင်း') {
                    // Save menu item without photo
                    try {
                        ...
                        await ctx.reply(`✅ Menu item "${ctx.session.menuData.item_name}" ထည့်ပြီးပါပြီ!`);
                        
                        // Clear session
                        delete ctx.session.menuStep;
                        delete ctx.session.menuData;
                    ...
                    await ctx.reply('❌ ဓာတ်ပုံပေးပို့ပါ သို့မဟုတ် "ခုံချင်း" ဟုရိုက်ပါ:');
                }
                break;
        }
    }

    // Handle search (for text messages)
    async handleSearch(ctx) {
        const searchText = ctx.message.text.trim();
        
        // Try to find by township first
        let shops = await database.getShopsByTownship(searchText);
        
        if (shops.length === 0) {
            // Try to find by vibe
            shops = await database.getShopsByVibe(searchText);
        }
        
        if (shops.length === 0) {
            await ctx.reply('❌ ရှာလို့မရပါ။ မြို့နယ် သို့မဟုတ် vibe အမည်ကိုပြန်ရိုက်ပါ။');
            return;
        }

        let message = `🔍 *${searchText} ရလဒ်များ*\n\n`;
    ...
    // Handle session (for multi-step wizards)
    async handleSession(ctx) {
        if (ctx.session.shopStep) {
            await this.handleShopCreation(ctx);
        } else if (ctx.session.menuStep) {
            await this.handleMenuCreation(ctx);
        } else if (ctx.session.townshipStep) {
            await this.handleTownshipCreation(ctx);
        }
    }
    ...
    async viewFavorites(ctx) {
        ...
        if (favorites.length === 0) {
            await ctx.reply('⭐ *အကြိုက်ဆုံးများ*\n\nသင့်အကြိုက်ဆုံးများ မရှိပါ။');
            return;
        }

        let message = '⭐ *အကြိုက်ဆုံးများ*\n\n';
        
        favorites.forEach((favorite, index) => {
            message += `${index + 1}. *${favorite.shop_name}*\n`;
            message += `   📍 ${favorite.township}\n\n`;
        });
    ...
    async randomRecommendation(ctx) {
        await this.surpriseMe(ctx);
    }

    // Add review command
    async addReview(ctx) {
        await ctx.reply('⭐ *သုံးသပ်ချက်ရေးရန်*\n\nဆိုင်ရွေးချယ်ပြီး rating ပေးလိုက်ပါ။');
    }
